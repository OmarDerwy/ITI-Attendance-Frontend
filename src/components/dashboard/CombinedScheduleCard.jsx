  ...
  const fetchScheduleData = async (trackId) => {
    try {
      ...
      const upcomingEvents = allEvents.filter(
        (event) =>
          isAfter(event.start, currentTime) && isBefore(event.start, nextDays)
      );
        ...
        <CardContent className="pt-5">
          ...
          <div className="space-y-3 mt-4">
            {todayClasses.length > 0 ? (
              todayClasses.map((cls) => (
                <div
                  ...
                  className={cn(
                    ...
                    cls.status === "active" &&
                      "bg-accent dark:bg-accent/80 shadow-sm",
                    cls.status === "upcoming" && "bg-card dark:bg-card"
                  )}
                >
                  ...
                  <div className="flex justify-between items-center mb-2">
                    <h3 className="font-semibold text-base pl-2 text-foreground">
                      {cls.title}
                    </h3>
                    {cls.status === "completed" && (
                      <span className="px-2.5 py-1 text-xs bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-full font-medium flex items-center">
                    ...
                    {cls.status === "active" && (
                      <span className="px-2.5 py-1 text-xs bg-primary/10 dark:bg-primary/20 text-primary dark:text-primary-foreground rounded-full font-medium flex items-center border border-primary/20">
                        In Progress
                      </span>
                    )}

                    {cls.status === "upcoming" && (
                      <span className="px-2.5 py-1 text-xs bg-card dark:bg-card text-primary dark:text-primary-foreground rounded-full font-medium flex items-center border border-primary/10">
                  ...
                  <div className="flex items-center gap-3 pl-2 mt-3">
                    ...
                    {cls.isOnline ? (
                      <div className="flex items-center gap-1.5 text-sm text-accent-foreground dark:text-red-300">
              ...
              <div className="text-center p-8 bg-accent/50 dark:bg-accent/20 rounded-lg border border-dashed border-primary/20">
                <div className="mx-auto w-16 h-16 rounded-full bg-accent dark:bg-accent/30 flex items-center justify-center mb-3">
                  <Calendar className="h-8 w-8 text-primary dark:text-primary-foreground" />
                </div>
                <p className="text-base text-primary-foreground dark:text-primary-foreground/90">
      ...
      <Card className="overflow-hidden h-full lg:col-span-6">
        <CardContent className="pt-4">
          <div className="flex justify-between items-center mb-3">
            ...
            <Link
              to="/student-schedule"
              className="text-sm text-primary dark:text-primary-foreground hover:text-primary/80 dark:hover:text-primary-foreground/80 flex items-center font-medium"
            >
                ...
                {upcomingDays.map((day, dayIndex) => {
                  ...
                  return (
                    <div
                      key={day.dateStr}
                      className={cn(
                        "flex border-b dark:border-gray-800 last:border-b-0",
                        day.isTomorrow
                    ...
                    >
                      ...
                      <div
                        className={cn(
                          "w-[90px] p-2 border-r dark:border-gray-800 flex flex-col justify-center shrink-0",
                          day.isTomorrow &&
